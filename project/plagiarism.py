...
def dijkstra_shortest_path(grid_obs, source, dest):
    ...
    #   Fill and submit this code
    #
    action_trans = [-21, 21, -1, 1]
    listy = []  #(location, list so far)
    completed = []
    listy.append([source, []])
    cur = -1
    while(cur != dest):
        ...
        if cur not in completed:
            ...
            for act in action_trans:
                curTrying = act + cur
                listSoFar = curNode[1].copy()
                listSoFar.append(curTrying)
                listy.append([curTrying, listSoFar])
